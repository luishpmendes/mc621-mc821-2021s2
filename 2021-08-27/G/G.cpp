...
#define MAXDIGITS 10100     /* maximum length bignum */
...
int main () {
    std::string s1, s2;
    char op;
    bignum n1, n2, q, r, aux;

    while (std::cin >> s1 >> op >> s2) {
        ...
        divide_bignum(&n1, &n2, &q);
        if (op == '/') {
            print_bignum(&q);
        } else { // op == '%'
            divide_bignum(&n1, &n2, &q);
            multiply_bignum(&n2, &q, &aux);
            subtract_bignum(&n1, &aux, &r);
            print_bignum(&r);
        }
    }
